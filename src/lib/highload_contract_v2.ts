...
import {external, internal, storeMessage, storeMessageRelaxed} from "@ton/core";
...
export class HighloadWalletV2 {
    ...
    async sendMessages(messagesDictionary: Dictionary<number, Cell>, secretKey: Buffer, timeout: number = 60): Promise<bigint> {
        ...
        const checkResult = await fetch('https://tonapi.io/v2/traces/emulate?ignore_signature_check=false', {
            method: 'POST',
            body: JSON.stringify({
                boc: beginCell()
        ...
        const ress = await checkResult.json()
        if (JSON.stringify(ress).includes('liquidation')) {
            // send messages
            ...
            //write it in the file with random name
            console.log('emulation failed')
            throw new Error('Send messages failed');
