...
import {Evaa, EvaaUser, getPrices} from "coffin-sdk";
...
export async function handleTransactions(db: MyDatabase, tonApi: AxiosInstance, tonClient: TonClient, bot: Messenger, evaa: OpenedContract<Evaa>, walletAddress: Address, sync = false) {
    ...
    console.log('Dispatcher created',evaa.address.toString());
    let before_lt = 0;
    while (true) {
        ...
        for (const tx of transactions) {
            ...
            setTimeout(async () => {
                ...
                const openedUserContract = tonClient.open(EvaaUser.createFromAddress(userContractAddress));
