...
import {Evaa, FEES, JettonLiquidationParameters, TonLiquidationParameters} from "coffin-sdk";
import {ASSET_ID, JETTON_WALLETS, LIQUIDATION_BALANCE_LIMITS, POOL_CONFIG} from "../../config";
...
export async function handleLiquidates(db: MyDatabase, tonClient: TonClient,
    ...
    const assetIds = POOL_CONFIG.poolAssetsConfig
        .filter(asset => asset.assetId !== ASSET_ID.TON)
    ...
    for (const task of tasks) {
        ...
        if (assetAmount < task.liquidation_amount) {
            ...
            task.liquidation_amount = (task.loan_asset === ASSET_ID.TON) ? assetAmount - toNano(1) : assetAmount;
        ...
        let amount = 0n;
        let destAddr: string;
        let liquidationParams: TonLiquidationParameters | JettonLiquidationParameters;
        if (task.loan_asset === ASSET_ID.TON) {
            liquidationParams = {
